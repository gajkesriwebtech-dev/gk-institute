...
export const DashboardLayout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Clean, minimal dashboard sidebar for purchased users
  const navItems: NavItem[] = [
    { label: 'My Course', href: '/dashboard', icon: <Icons.Book /> },
    { label: 'Profile', href: '/dashboard/profile', icon: <Icons.User /> },
    { label: 'Support', href: '/dashboard/support', icon: <Icons.HelpCircle /> },
  ];

  return (
    <div className="min-h-screen bg-white dark:bg-slate-950 flex transition-colors duration-300">
      <aside className="hidden md:flex md:w-64 md:flex-col fixed inset-y-0 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 z-30 shadow-sm transition-colors duration-300">
        <div className="flex flex-col flex-grow pt-6 pb-4 overflow-y-auto">
          <div className="flex items-center flex-shrink-0 px-6 mb-10 cursor-pointer" onClick={() => { navigate('/'); window.scrollTo({ top: 0, behavior: 'smooth' }); }}>
            <BrandLogo />
          </div>
          <div className="flex-1 flex flex-col space-y-1.5 px-3">
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  <span className={`mr-3 flex-shrink-0 transition-colors ${isActive ? 'text-brand-600 dark:text-brand-400' : 'text-slate-400 dark:text-slate-500 group-hover:text-slate-500 dark:group-hover:text-slate-300'}`}>
                    {item.icon}
                  </span>
                  {item.label}
                </button>
          ...
          <div className="px-3 pb-4 border-t border-slate-100 dark:border-slate-800 pt-4">
            <button
              onClick={() => navigate('/')}
              className="w-full flex items-center px-3 py-2.5 text-sm font-medium text-slate-600 dark:text-slate-400 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 hover:text-red-600 dark:hover:text-red-400 transition-colors"
            >
              <span className="mr-3 flex-shrink-0 text-slate-400 group-hover:text-red-500 dark:group-hover:text-red-400">
                <Icons.LogOut />
              </span>
              Sign Out
            </button>
      ...
      <div className="md:hidden fixed top-0 left-0 right-0 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 z-20 h-16 flex items-center justify-between px-4 transition-colors duration-300">
        <BrandLogo />
        <div className="flex items-center gap-4">
          <ThemeToggle />
          <button className="text-slate-500 dark:text-slate-400"><Icons.Menu /></button>
        </div>
      </div>
      <div className="flex-1 md:pl-64 flex flex-col min-h-screen">
        <header className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 h-16 flex items-center justify-between px-4 sm:px-8 sticky top-0 z-10 shadow-sm transition-colors duration-300">
          <div className="flex items-center text-slate-400 text-sm">
            <span className="hover:text-slate-600 dark:hover:text-slate-200 cursor-pointer transition-colors" onClick={() => navigate('/dashboard')}>Student Area</span>
            <span className="mx-2">/</span>
            <span className="font-semibold text-slate-900 dark:text-white capitalize">
              {pathname.includes('profile') ? 'Profile' : pathname.includes('support') ? 'Support' : 'My Curriculum'}
            </span>
          </div>
          <div className="flex items-center space-x-6">
            ...
            <button className="p-2 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 relative transition-colors">
              <Icons.Bell />
              <span className="absolute top-2 right-2.5 w-2 h-2 bg-red-500 rounded-full border-2 border-white dark:border-slate-900"></span>
            </button>
            <div className="flex items-center gap-3 cursor-pointer p-1 rounded-full hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors pr-2">
              <div className="h-9 w-9 rounded-full bg-brand-100 dark:bg-brand-900/50 flex items-center justify-center text-brand-700 dark:text-brand-300 font-bold text-sm border border-brand-200 dark:border-brand-800">
                ST
              </div>
              <div className="hidden sm:block text-left">
                <div className="text-xs font-bold text-slate-900 dark:text-white">Student</div>
              </div>
            </div>
          </div>
        </header>
        <main className="flex-1 p-4 sm:p-8 overflow-y-auto bg-white dark:bg-slate-900 transition-colors duration-300">
