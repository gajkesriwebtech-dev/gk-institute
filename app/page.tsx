...
const LandingPage = () => {
    ...
    return (
        <PublicLayout>
            ...
            {/* SECTION: Plan Your Learning Investment Banner 
            <section className="py-16 bg-white dark:bg-slate-900 border-y border-slate-100 dark:border-slate-800">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="flex flex-col lg:flex-row items-center justify-between gap-8 bg-slate-50 dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 px-8 py-10">
                        <div className="flex items-start gap-5">
                            <div className="w-14 h-14 rounded-xl bg-slate-100 dark:bg-slate-900 flex items-center justify-center text-2xl flex-shrink-0">
                                🧮
                            </div>
                            <div>
                                <p className="text-xs uppercase tracking-widest font-black text-[#1F4037] dark:text-emerald-500 mb-1">Education Investment Planner</p>
                                <h2 className="text-xl font-black text-slate-900 dark:text-white leading-snug uppercase">
                                    Not sure which courses to invest in?
                                </h2>
                                <p className="text-slate-500 dark:text-slate-400 text-sm mt-1.5 max-w-xl leading-relaxed font-medium">
                                    Use our pricing simulator to plan your exact learning path — choose courses, durations, and add-ons to see a real-time investment breakdown before you commit.
                                </p>
                                <div className="flex flex-wrap gap-4 mt-4 text-xs text-slate-500 dark:text-slate-400 font-bold">
                                    <span className="flex items-center gap-1.5"><span className="text-green-500">✓</span> Bundle discounts automatically applied</span>
                                    <span className="flex items-center gap-1.5"><span className="text-green-500">✓</span> Promo &amp; referral code support</span>
                                    <span className="flex items-center gap-1.5"><span className="text-green-500">✓</span> No payment required to plan</span>
                                </div>
                            </div>
                        </div>
                        <div className="flex-shrink-0">
                            <Button
                                variant="marigold"
                                size="lg"
                                className="px-10 whitespace-nowrap shadow-xl"
                                onClick={() => router.push('/plan-learning')}
                            >
                                Plan My Investment →
                            </Button>
                        </div>
